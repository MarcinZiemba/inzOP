...
public:
    // (Singleton)
    static Lodowka& getInstance() {
        static Lodowka instance(1); // Tworzenie instancji tylko raz, przy pierwszym wywołaniu
        return instance;
    }
...
private:
    // Prywatny konstruktor, aby uniemożliwić tworzenie instancji poza klasą
    Lodowka(int _id_lodowki) : id_lodowki(_id_lodowki) {}

    // Prywatny destruktor, aby uniemożliwić usuwanie instancji poza klasą
    ~Lodowka() {}

    // Prywatne pole przechowujące produkty w lodówce
    std::vector<Produkt> produkty;
    int id_lodowki;
};
